...
completeZeros_sp <- function(df, species, year){
  ...
  df %>%
    ...
    tidyr::complete(nesting(aphiaid, scientificnameaccepted),         # these will be completed, with their occurrence
                    nesting(datecollected, xUTM, yUTM, season, eventid),   # Combinations of these parameters are to be found
...
save(phy_ts_eff, file = "../data/derived_data/phy_ts_eff.Rdata")
save(phy_effort, file = "../data/derived_data/phy_effort.Rdata")
save(phy_effort_season, file = "../data/derived_data/phy_effort_season.Rdata")
```


```{r functions needed for plotting, include=FALSE, warning=FALSE}
# Determine height for first plot
height_plot1 <- (2+0.5*length(unique(phy_ts_eff$datasetID))) # Size of height plot 1
...
```{r plot2a, echo=FALSE, warning = FALSE, dpi = 184}
