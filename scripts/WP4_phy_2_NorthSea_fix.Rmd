...
fix_phy <- clean_phy %>%
  ...
  mutate(datasetid_new = str_replace(datasetID, ".csv", "")) %>%                                                     # Datasetid has ".csv" at end
  select(-datasetID, -datasetid) %>%                                   
  mutate(datasetID = as.numeric(datasetid_new)) %>%                                                                  # We remove this here
  ...
  select(-eventid, -eventID_2, -coordinateuncertaintyinmeters, -aphiaidaccepted, -aphiaid, -datasetid_new) %>%       # Remove columns not necessary
