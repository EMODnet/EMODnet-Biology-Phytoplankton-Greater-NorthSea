...
params:
  targetSpecies: "Acanthostomella norvegica"
  begin: 1995
  end: 2020
  gridX: 15000
  gridY: 15000
...
phy_sp <- phy_sp %>%
  select(aphiaid, scientificnameaccepted, datecollected, datasetID, datasetid, mrgid, eventid, month, year, season, xUTM, yUTM, occurrence) 
...
completeZeros_sp <- function(df, species, year){
  ...
  df %>%
    ...
    tidyr::complete(nesting(aphiaidaccepted, scientificnameaccepted),         # these will be completed, with their occurrence
                    nesting(datecollected, xUTM, yUTM, season),   # Combinations of these parameters are to be found
                    fill = list(present = 0)) %>% 
    ungroup() %>%
    unite(date_xUTM_yUTM, datecollected, xUTM, yUTM, remove = FALSE) %>%
...
time_effort <- function(df) {
  df %>% ungroup() %>%
    ...
    summarize(effort_per_season = n(), positives = sum(present == 1)) %>%
...
selectedDatasets <- phy_sp %>% 
    ungroup() %>%
    dplyr::filter(scientificnameaccepted == "Acanthostomella norvegica") %>%
    distinct(datasetID) %>% unlist() %>% unname()
...
phy_c <- phy_sp %>%
    ...
    ungroup() %>%
    unite(date_xUTM_yUTM, datecollected, xUTM, yUTM, remove = FALSE) %>%
    filter(scientificnameaccepted == "Acanthostomella norvegica") %>%
    ungroup()
