...
phy_sp <- phy_sp %>%
  distinct(abbr, aphiaID, scientificName, date, xUTM, yUTM, date_year, season, occurs)
...
dup_zero <- phy_c %>% 
  arrange(aphiaID, date, xUTM, yUTM, occurs, season) %>%
  select(-abbr) %>%
  duplicated %>% which
...
phy_c <- phy_c %>%
  distinct(aphiaID, scientificName, date, xUTM, yUTM, date_year, season, occurs, .keep_all = TRUE) 
...
height_plot1 <- (2+0.5*length(unique(phy_c$abbr))) # Size of height plot 1
