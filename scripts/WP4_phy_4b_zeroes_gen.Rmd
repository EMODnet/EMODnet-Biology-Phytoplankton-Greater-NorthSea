...
dup_zero_g <- phy_c_g %>% 
  arrange(genus, datecollected, xUTM, yUTM, occurrence, season) %>%
  select(-datasetID) %>%
  duplicated %>% which
...
phy_c_g <- phy_c_g %>%
  distinct(genus, datecollected, xUTM, yUTM, date_year, season, occurrence, .keep_all = TRUE)
...
```{r plot2a, echo=FALSE, warning = FALSE, fig.width = 15, dpi = 184}
...
```{r plot2b, echo=FALSE, warning = FALSE, fig.width = 15, dpi = 184}
