...
# Genus datasets
phy_gen <- phy %>%
   distinct(genus, datasetID, date, xUTM, yUTM, .keep_all = TRUE) # Here we remove the duplicates that originated due to focussing on genera alone
...
# bbox
# phy_gen %>% sf::st_as_sf(coords = c('xUTM','yUTM'), crs = 32631) %>% sf::st_transform(4326) %>% sf::st_bbox()
