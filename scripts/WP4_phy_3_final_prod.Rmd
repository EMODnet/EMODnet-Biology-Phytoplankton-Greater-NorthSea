...
temp_phy <- fix_phy %>%
  ...
  mutate(occurrence = "1") %>%
  separate(scientificnameaccepted, c("genus_new", "species"), " ", remove = FALSE) %>%
  select(-genus_new)
...
phy_sp <- phy %>%
  ...
  distinct(aphiaid, datasetID, scientificnameaccepted, datecollected, eventid, xUTM, yUTM, year,season, occurrence, .keep_all = TRUE) 

# Genus datasets
phy_gen <- phy %>%
   distinct(genus, datasetID, datecollected, xUTM, yUTM, .keep_all = TRUE) # Here we remove the duplicates that originated due to focussing on genera alone
